...
plot_crossection <- function(las_NN55NW,
                             p1 = c(min(las_NN55NW@data$X), mean(las_NN55NW@data$Y)),
                             p2 = c(max(las_NN55NW@data$X), mean(las_NN55NW@data$Y)),
                             width = 4, colour_by = NULL)
{
  colour_by <- enquo(colour_by)
  data_clip <- clip_transect(las_NN55NW, p1, p2, width)
...
plot_crossection(las_NN55NW, colour_by = factor(Classification)) #Classification information here: https://desktop.arcgis.com/en/arcmap/latest/manage-data/las-dataset/lidar-point-classification.htm 
...
dtm_idw_NM8428 <- rasterize_terrain(las_NM8428, algorithm = knnidw(k = 10L, p = 2));beep() #try using the invert distance weighting as test (medium in terms of edge effects, but has big steps in the dtm but so may not be suitable for bunloit considering the terrain of plateaus down to the loch)
plot_dtm3d(dtm_idw_NM8428, bg = "white")

#######Use the DTM to normalize the point cloud#########
#Using Subtraction of the dtm
nlas_NM8428_sub <- las_NM8428 - dtm_idw_NM8428 #this method is less accurate but simpler and less computationally heavy
plot(nlas_NM8428_sub, bg = "white")

#Normalizing the point cloud through interpolation
nlas_NM8428_int <- normalize_height(las_NM8428, knnidw()) #more accurate method, but more computationally heavy and also suffers from edge effects
...
chm_ptr_NM8428 <- rasterize_canopy(las_NM8428, res = 1, algorithm = p2r())
col <- height.colors(25)
plot(chm_ptr_NM8428, col = col)

#Triangulation method
chm_tri_NM8428 <- rasterize_canopy(las_NM8428, res = 1, algorithm = dsmtin(max_edge = 8));beep() #max_edge removes things like lochs, becuase it won't make triangles longer than 8
plot(chm_tri_NM8428, col = col)
...
#Calculate the rumple_index. Comuptes the roughness of the surface. Higher Values mean a rougher surface
